      ...
      <v-row class="flex-nowrap">
        ...
        <v-col id="attrCol" ref="attrCol" cols="auto">
          ...
          <v-row class="pb-3" align="center">
            ...
            <!-- LABEL -->
            <v-col class="attr-col-header-table" cols="auto">
              <span>primary.csv</span>
            </v-col>
          </v-row>
          ...
          <v-row
            ...
            align="center"
            v-for="(value, key) in unAddedToAttributesPrimaryAttributes"
            :key="key"
          >
            <!-- HAS RELATED ATTRIBUTES -->
            <template v-if="hasRelatedAttributes(key)">
              <v-menu
                bottom
                max-height="400px"
                :offset-y="true"
              ...
              >
                <template v-slot:activator="{ on, attrs }">
                  ...
                  >
                    <v-row
                      class="attr-col-card-menu-content"
                      align="center"
                      v-on="on"
                      v-bind="attrs"
                    >
                      ...
                      <v-icon
                        :color="
                          utils.getDataTypeIconAndColor(value.dataType).color
                        "
                        v-text="
                          utils.getDataTypeIconAndColor(value.dataType).icon
                        "
                      ></v-icon>
                      ...
                      <span
                        ...
                        :style="{
                          color: utils.getDataTypeIconAndColor(value.dataType)
                            .color,
                        }"
                        v-text="key"
                      ></span>
                ...
                <v-list dense>
                  <!-- NO UNADDED RELATED ATTRIBUTES -->
                  <template
                    v-if="
                      getUnAddedToAttributesRelatedAttributes(value).length == 0
                    "
                  >
                    <v-list-item>
                      <v-list-item-content>
                        <v-list-item-title>
                          All related attributes added!
                        </v-list-item-title>
                      </v-list-item-content>
                    </v-list-item>
                  </template>

                  <!-- UNADDED RELATED ATTRIBUTES REMAINING -->
                  <template v-else>
                    <v-list-item
                      v-for="(
                        item, index
                      ) in getUnAddedToAttributesRelatedAttributes(value)"
                      :key="index"
                      :style="{ height: '40px' }"
                      @click="addAttrToAttributesPanel(item)"
                    >
                      <v-list-item-icon>
                        <v-icon
                          :color="
                      ...
                      <v-list-item-content>
                        ...
                        <span
                          v-if="isSecondaryAttribute(item)"
                          class="source-table-subtext"
                          v-text="attributes[item].source"
                        ></span>
                      </v-list-item-content>

                      <v-list-item-icon v-if="isSecondaryAttribute(item)">
            ...
            <!-- DOES NOT HAVE RELATED ATTRIBUTES -->
            <template v-else>
              <v-col
                :style="{
                  'background-color': isEncoded(value) ? '#1976d2' : null,
          ...
          <!-- ADDED ATTRIBUTES -->
          <template v-if="Object.keys(addedToAttributes).length > 0">
            <!-- DIVIDER -->
            ...
            <v-row
              ...
              align="center"
              v-for="(value, key) in addedToAttributes"
              :key="key"
            >
              <v-col
                :style="{
                ...
                <v-col cols="7">
                  ...
                  <v-row class="pb-6">
                    <v-select
                      ...
                      hide-details
                      clearable
                      ref="xAxisAttributeSelect"
                      label="X-axis Attribute"
                      v-model="encodings.xVar"
                      :items="encodableAttributes"
                      :menu-props="{ bottom: true, offsetY: true }"
                      ...
                      <template v-slot:item="{ item, on, attrs }">
                        <v-list-item
                          :style="{
                            height: '40px',
                            'border-bottom':
                              Object.keys(secondaryAttributes).length > 0 &&
                              encodableAttributes.indexOf(item) ==
                                Object.keys(primaryAttributes).length - 1
                                ? '2px solid #C4C4C4'
                                : '',
                          }"
                          v-on="on"
                      ...
                      <template v-slot:item="{ item, on, attrs }">
                        <v-list-item
                          :style="{
                            height: '40px',
                            'border-bottom':
                              Object.keys(secondaryColorAttributes).length >
                                0 &&
                              colorAttributes.indexOf(item) ==
                                Object.keys(primaryColorAttributes).length - 1
                                ? '2px solid #C4C4C4'
                                : '',
                          }"
                          v-on="on"
                ...
                <v-menu bottom :offset-y="true">
                  ...
                  <v-card style="max-height: 304px">
                    <v-list dense>
                      <template
                        v-if="
                          inactiveFilterPrimaryAttributes.concat(
                            inactiveFilterSecondaryAttributes
                          ).length > 0
                        "
                      >
                        <!-- INACTIVE FILTER ON PRIMARY ATTRIBUTES -->
                        <v-list-item
                          v-for="attr in inactiveFilterPrimaryAttributes"
                          :key="attr"
                        ...
                        >
                          ...
                          <v-list-item-content>
                            ...
                            ></v-list-item-title>
                            <span
                              v-if="isSecondaryAttribute(attr)"
                              class="source-table-subtext"
                              v-text="attributes[attr].source"
                            ></span>
                          </v-list-item-content>

                          <v-list-item-icon v-if="isSecondaryAttribute(attr)">
                            <v-icon>mdi-link</v-icon>
                          </v-list-item-icon>
                        </v-list-item>
                        ...
                        <v-divider
                          v-if="
                            inactiveFilterPrimaryAttributes.length > 0 &&
                            inactiveFilterSecondaryAttributes.length > 0
                          "
                          style="border-top-width: 2px; border-color: #c4c4c4"
                        ></v-divider>

                        <!-- INACTIVE FILTER ON SECONDARY ATTRIBUTES -->
                        <v-list-item
                          v-for="attr in inactiveFilterSecondaryAttributes"
                          :key="attr"
                          @click="addFilter(attr)"
                        >
                          <v-list-item-icon>
              ...
              <v-row class="flex-nowrap pt-3">
                ...
                >
                  ...
                  <v-row class="elaborate-table-row-header">
                    <!-- HAS RELATED ATTRIBUTES -->
                    <template v-if="hasRelatedAttributes(header.value)">
                      <v-menu
                        bottom
                        max-height="400px"
                        :offset-y="true"
                      ...
                      >
                        <template v-slot:activator="{ on, attrs }">
                          <v-col
                            class="pa-1 elaborate-table-row-header-menu"
                            :style="{
                              'background-color': isEncoded(
                        ...
                        <v-list dense>
                          ...
                          <template
                            v-if="
                              getUnAddedToElaborateRelatedAttributes(
                                attributes[header.value]
                              ).length == 0
                            "
                          >
                          ...
                          <template v-else>
                            <v-list-item
                              v-for="(
                                item, index
                              ) in getUnAddedToElaborateRelatedAttributes(
                                attributes[header.value]
                              )"
                              :key="index"
                              :style="{ height: '40px' }"
                              @click="addAttrToElaboratePanel(item)"
                            >
                              <v-list-item-icon>
                                <v-icon
                                  :color="
                    ...
                    <template v-else>
                      ...
                      >
                        <v-row class="pa-3">
                          ...
                          <!-- ICON -->
                          <v-icon v-if="isSecondaryAttribute(header.value)">
                            mdi-link
                          </v-icon>
                        </v-row>
                      </v-col>
                ...
                <template v-if="addedETblHeaders.length > 0">
                  <!-- DIVIDER -->
                  ...
                  <v-col
                    ...
                    cols="auto"
                    v-for="header in addedETblHeaders"
                    :key="header.value"
                  >
                    <!-- HEADER ROW -->
                    <v-row class="elaborate-table-row-header">
...
export default {
  ...
  computed: {
    ...
    primaryAttributes() {
      return Object.fromEntries(
        Object.entries(this.attributes).filter(([, value]) => value.primary)
      );
    },
    ...
    addedToAttributes() {
      return Object.fromEntries(
        Object.entries(this.attributes).filter(
          ([, value]) => value.addedToAttributes
        )
      );
    },

    unAddedToAttributesPrimaryAttributes() {
      return Object.fromEntries(
        Object.entries(this.attributes).filter(
          ([, value]) => !value.addedToAttributes && value.primary
        )
      );
    },
    ...
    addedToElaborate() {
      return Object.fromEntries(
        Object.entries(this.attributes).filter(
          ([, value]) => value.addedToElaborate
        )
      );
    },

    /**
     * Ordered list of attribute names based on primary status of attribute
     */
    encodableAttributes() {
      return Object.keys(this.primaryAttributes).concat(
        Object.keys(this.secondaryAttributes)
      );
    },
    ...
    primaryColorAttributes() {
      return this.colorAttributes.filter((x) => this.attributes[x].primary);
    },

    secondaryColorAttributes() {
      return this.colorAttributes.filter((x) => !this.attributes[x].primary);
    },
    ...
    inactiveFilterPrimaryAttributes() {
      return Object.keys(this.primaryAttributes).filter(
        (attr) => this.activeFilterAttributes.indexOf(attr) == -1
      );
    },

    inactiveFilterSecondaryAttributes() {
      return Object.keys(this.secondaryAttributes).filter(
        (attr) => this.activeFilterAttributes.indexOf(attr) == -1
      );
    },

    eTblHeaders() {
      return Object.keys(this.primaryAttributes).map((attr) => {
        return { text: attr, value: attr };
      });
    },

    addedETblHeaders() {
      return Object.keys(this.addedToElaborate).map((attr) => {
        return { text: attr, value: attr };
      });
    },
  ...
  methods: {
    init() {
      ...
      vm.loading = false; // let the interface show!
    },

    isSecondaryAttribute(attr) {
      return this.attributes[attr].secondary;
    },

    hasRelatedAttributes(attr) {
      return this.attributes[attr].relatedAttributes.length > 0;
    },
    ...
    getUnAddedToAttributesRelatedAttributes(value) {
      return value.relatedAttributes.filter(
        (attr) => Object.keys(this.addedToAttributes).indexOf(attr) == -1
      );
    },

    getUnAddedToElaborateRelatedAttributes(value) {
      return value.relatedAttributes.filter(
        (attr) =>
          Object.keys(this.addedToAttributes).indexOf(attr) == -1 &&
          Object.keys(this.addedToElaborate).indexOf(attr) == -1
      );
    },

    addAttrToAttributesPanel(attr) {
      let vm = this;
      vm.taskData.secondaryAttributesAddedToAttributesPanel.total += 1;
      vm.taskData.secondaryAttributesAddedToAttributesPanel.added.push(attr);
      vm.$set(vm.attributes[attr], "primary", true);
      vm.$set(vm.attributes[attr], "addedToAttributes", true);
      vm.$set(vm.attributes[attr], "addedToElaborate", false);
      vm.$root.$data.attributes = vm.attributes;
      vm.$forceUpdate();
    },
    ...
    updateXAxisAttr() {
      ...
      if (vm.attributes[attr].primary) {
        vm.taskData.primaryAttributesAddedToEncodePanel.total += 1;
        vm.taskData.primaryAttributesAddedToEncodePanel.added.push(attr);
      } else if (vm.attributes[attr].secondary) {
    ...
    addFilter(attr) {
      ...
      if (index == -1) {
        ...
        if (vm.attributes[attr].primary) {
          vm.taskData.primaryAttributesAddedToFilterPanel.total += 1;
          vm.taskData.primaryAttributesAddedToFilterPanel.added.push(attr);
        } else if (vm.attributes[attr].secondary) {
    ...
    addAttrToElaboratePanel(attr) {
      this.taskData.secondaryAttributesAddedToElaboratePanel.total += 1;
      this.taskData.secondaryAttributesAddedToElaboratePanel.added.push(attr);
      this.$set(this.attributes[attr], "addedToElaborate", true);
      this.$forceUpdate();
    },
...
.attr-col-card-menu-content {
  padding: 4px;
}

.attr-col-card-menu-content:hover {
...
.elaborate-table-row-header-menu:hover {
